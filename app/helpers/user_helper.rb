...
module UserHelper
  def profile_pic_url(user, options)
    ...
    if user && !anonymous && profile_pic = user.profile_picture
      ...
      user_gender = user.nil? ? 'male' : (user.gender || 'male')
      uid         = app.store(Pathname.new(Rails.application.config.assets.paths.first + "/placeholder-#{user_gender}-#{Random.rand(1..3)}.jpg"))
      image       = app.fetch(uid)
